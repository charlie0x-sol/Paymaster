...
export class FeeEstimator {
  ...
  private cacheDurationMs: number = 2000;
  private lastFetchTime: number = 0;
  private cachedFee: number = 0;
   ...
   */
  async getPriorityFeeEstimate(lookbackSlots: number = 150): Promise<number> {
    const now = Date.now();
    ...
    try {
      ...
      if (this.cachedFee > 1000000) {
          logger.warn(`Fee estimate suspiciously high: ${this.cachedFee}, capping at 1000000`);
      ...
      logger.error('Error fetching prioritization fees', { error: error.message });
